...
public class ScratchStage extends ScratchObj {
	...
	public function collectBroadcastVarNames(msg:String):Array {
		...
		// format BROADCAST_VAR + msg + '>' + (some name).
		//
		...
		for each (var v:Variable in variables) {
			var n:String = v.name;
			if (n.indexOf(Specs.BROADCAST_VAR) === 0) {
				var len:int = Specs.BROADCAST_VAR.length;
				if (n.indexOf(msg + Specs.SCRAP_VAR_SEPARATOR, len) === len) {
					result.push([
